...
- `drizzle/2026-05-14-subscriptions.sql` — tabla `subscriptions` (id, place_id FK, tier CHECK 'featured', status CHECK active/expired/canceled, amount_clp, provider CHECK manual/khipu/stripe, external_id, current_period_start/end, created_by FK users, notes, canceled_at, timestamps). Índices: `(place_id)`, `(status, current_period_end)` pa cron expire, partial UNIQUE `(place_id, tier) WHERE status='active'` invariante. MVP monetización Fase 1. Idempotente. Aplicada en Neon prod ✅.
- `drizzle/2026-05-14-premium-tier.sql` — Fase 2 monetización. ALTER constraint tier en subscriptions pa sumar 'premium'. Tabla `place_events` (id, place_id FK, event_type CHECK view/contact_click, channel CHECK whatsapp/instagram/website/maps/phone, visitor_id, created_at) + índices `(place_id, created_at DESC)` y `(created_at DESC)` pa cleanup retention. Tabla `review_replies` (review_id PK FK reviews, place_id FK, author_id FK users, text, timestamps) + índice `(place_id)`. Idempotente. Aplicada en Neon prod ✅.
- `drizzle/2026-05-15-brands.sql` — Cadenas. Tabla `brands` (id, slug UNIQUE, name, logo_url, color, website, is_active, timestamps). `places.brand_id` FK opcional ON DELETE SET NULL + índice. Habilita pin map con logo de cadena. Idempotente. Aplicada en Neon prod ✅.
- `drizzle/2026-05-15-brand-subs.sql` — Subscriptions a nivel cadena. `subscriptions.place_id` ahora nullable, agrega `brand_id` FK CASCADE. CHECK XOR `(place_id IS NULL) <> (brand_id IS NULL)`. Partial unique indices: `(place_id, tier) WHERE status='active' AND place_id IS NOT NULL` y `(brand_id, tier) WHERE status='active' AND brand_id IS NOT NULL`. Sub a brand propaga `is_featured` a todos sus places. Idempotente. Aplicada en Neon prod ✅.
- `drizzle/2026-05-15-promotions.sql` — Ofertas (% descuento / producto destacado / combo). ALTER tier check pa sumar 'promo'. Tabla `promotions` (id, place_id FK CASCADE, kind CHECK, title, description, discount_pct CHECK 1-99, photo_url, region_label denorm, starts_at, ends_at, is_active, moderation_status CHECK pending/approved/rejected default approved, created_by, timestamps). Índices `(place_id)`, `(region_label, is_active, ends_at)` pa carousel home, `(ends_at)` pa cron expire. Idempotente. Aplicada en Neon prod ✅.
- `drizzle/2026-05-15-place-socials.sql` — `places.facebook` text + `places.tiktok` text + `places.menu_url` text. Suma redes sociales adicionales + link al menú digital pa render de buttons en ficha. Idempotente (3 ALTER ADD COLUMN IF NOT EXISTS). Aplicada en Neon prod ✅.
